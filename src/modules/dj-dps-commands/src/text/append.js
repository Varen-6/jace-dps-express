...
module.exports = {
  ...
  execute: function (command, state, config) {
    if (state.head.type != "string" && state.head.type != "html")
      throw new AppendImplError(
        "Incompatible context type: '" + state.head.type + "'."
      );
    if (!_.isString(state.head.data))
      throw new AppendImplError(
        "Incompatible context type: '" + typeof state.head.data + "'."
      );

    try {
      command.settings.value = _.isString(command.settings.value)
