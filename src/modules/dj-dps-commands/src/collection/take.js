...
module.exports = {
  ...
  execute: function (command, state, config) {
    ...
    if (
      command.settings.range &&
      _.isArray(command.settings.range) &&
      command.settings.range.length == 2
    ) {
    ...
    try {
      state.head = {
        data: state.head.data.filter(
          (v, idx) =>
            idx >= command.settings.start - 1 &&
            idx <= command.settings.stop - 1
        ),
        type: "json",
      };
    } catch (e) {
    ...
    params: [
      {
        name: "transform",
        synopsis:
          "javascript callback function(item){<return {key, value}>} via bindable (required)",
        type: ["bindable"],
        synonims: [],
        "default value": "none",
      },
