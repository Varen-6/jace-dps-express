  ...
  execute: function (command, state) {
    try {
      let getValue = (path) => {
        ...
        // if(!res) return null
        if (_.isArray(res)) return res.map((d) => d);
        if (_.isString(res)) return res;
        if (_.isObject(res)) return _.defaultsDeep({}, res);
        return res;
      };

      if (
        _.isUndefined(command.settings) ||
        _.isUndefined(command.settings.path) ||
        ...
        if (_.isFunction(state.head.data)) state.head.type = "function";
        return state;
      }
